...
for train, test in skf.split(X,y):
    ...
    clf_list = clf_rnn.fit(X_train_rnn, to_categorical(y[train]),epochs=40)
...
ytest_sum.to_csv('ytest_sum_LSTM_DNN_G_L_ST.csv')

yscore_=yscore[np.array(range(1,row)),:]
yscore_sum = pd.DataFrame(data=yscore_)
yscore_sum.to_csv('yscore_sum_LSTM_DNN_G_L_ST.csv')
...
data_csv.to_csv('LSTM_DNN_G_L_ST.csv')
