...
data_=pd.read_csv(r'Group_Lasso_FUusion_0.03_train.csv')
data=np.array(data_)
data=data[:,1:]
[m1,n1]=np.shape(data)
...
def get_CNNRNN_model(input_dim,out_dim):
    model = Sequential()
    model.add(Conv1D(filters = 32, kernel_size = 3, padding = 'same', activation= 'relu'))
    model.add(MaxPooling1D(pool_size=2,strides=1,padding="SAME"))
    model.add(Conv1D(filters = 32, kernel_size =  3, padding = 'same', activation= 'relu'))
    model.add(MaxPooling1D(pool_size=2,strides=1,padding="SAME")) 
    model.add(LSTM(int(input_dim/2), return_sequences=True,activation = 'relu'))
    model.add(Dropout(0.5))
    model.add(LSTM(int(input_dim/4), return_sequences=True,activation = 'relu'))
    model.add(Dropout(0.5))
    model.add(Flatten())
...
for train, test in skf.split(X,y):
    clf_rnn = get_CNNRNN_model(input_dim,out_dim)
    X_train_rnn=np.reshape(X[train],(-1,1,input_dim))
    X_test_rnn=np.reshape(X[test],(-1,1,input_dim))
    clf_list = clf_rnn.fit(X_train_rnn, to_categorical(y[train]),epochs=60)
...
ytest_sum.to_csv('ytest_sum_CNN_LSTM_G_L_ST.csv')

yscore_=yscore[np.array(range(1,row)),:]
yscore_sum = pd.DataFrame(data=yscore_)
yscore_sum.to_csv('yscore_sum_CNN_LSTM_G_L_ST.csv')
...
data_csv.to_csv('CNN_LSTM_G_L_ST.csv')
